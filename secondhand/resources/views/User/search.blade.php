...
@endsection
   ...
   <div id="hero-carousel" class="carousel slide" data-bs-ride="carousel">
    ...
    <a href="{{ route('User.detail', $item->title) }}" style="text-decoration: none;">
      ...
      <div class="card" data-name="{{$product->status}}">
       <div class="img">
         <div class="d-flex">
           <div class="status flex-fill">{{$product->status}}</div>
           <div class="sale flex-fill">{{$product->sale}}%</div>
         </div>
         @if ($product->thumbnail)
         <img src="{{ asset('assets/images/' . $product->thumbnail) }}" alt="Image">
         @else
             No Image
         @endif
       </div>
      ...
      <form  style="    height: 0.5rem;
          padding: 0;
          margin-top: -1.6rem;"  class="add" action="{{ route('cart.store') }}" method="POST">
            @csrf
